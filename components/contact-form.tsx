...
export function ContactForm({ className }: { className?: string }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const mailtoBody = [
      ...
      `Company: ${result.data.affiliation ?? 'n/a'}`,
    ...
    setStatus('success');
    setFeedback(
      "Thanks! Your email client should open. If it doesn't, email us at hello@corallodigital.com."
    );
    setFormState({
      name: '',
      email: '',
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Input
          ...
          placeholder="Company, product, research group, or individual"
        />
      </div>
      <div>
        <label
          htmlFor="message"
          className="mb-2 block text-xs font-semibold uppercase tracking-[0.3em] text-muted"
        >
          Message
        </label>
        <textarea
          ...
          required
          placeholder="Tell us why you're reaching out - product interest, partnership ideas, feedback, or general inquiries related to Corallo Digital."
        />
        {errors.message ? (
